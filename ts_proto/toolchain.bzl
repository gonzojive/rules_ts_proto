...
def _ts_proto_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "ts_proto_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    ts_protoinfo = ts_protoInfo(
...
ts_proto_toolchain = rule(
    implementation = _ts_proto_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a ts_proto compiler/runtime toolchain.
